...
namespace Gala
{
	...
	public class WindowManagerGala : Meta.Plugin, WindowManager
	{
        ...
        Clutter.DesaturateEffect? greyscale_effect;
        ...
        void toggle_greyscale ()
        {
            if (greyscale_effect == null) {
                greyscale_effect = new Clutter.DesaturateEffect (0.0f);
            ...
            if (AppearanceSettings.get_default ().greyscale) {
                ...
                fade_in_id = Timeout.add (FADE_TIMEOUT, () => {
                    bool stop = greyscale_effect.factor >= 1.0f;
                    if (stop) {
                        fade_in_id = 0U;
                    } else {
                        greyscale_effect.factor = double.min (greyscale_effect.factor + 0.01, 1.0);
                    }

                    print (greyscale_effect.factor.to_string () + "\n");
                    return !stop;
                });
            } else {
                if (ui_group.get_effect ("greyscale") != null) {
                    ...
                    fade_out_id = Timeout.add (FADE_TIMEOUT, () => {
                        bool stop = greyscale_effect.factor <= 0.0f;
                        if (stop) {
                            ui_group.remove_effect (greyscale_effect);
                            fade_out_id = 0U;
                        } else {
                            greyscale_effect.factor = double.max (greyscale_effect.factor - 0.01, 0);
                        }
